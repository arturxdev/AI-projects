![imagen logo](./images/logo.png)

# 🚀 Proyectos de IA para Practicar Tus Habilidades

Bienvenido a mi repositorio de proyectos prácticos de Inteligencia Artificial. Aquí encontrarás tutoriales paso a paso para construir aplicaciones reales de IA.

## 🎯 ¿Para quién es este repositorio?

Este repositorio está diseñado para personas que:

- 🎓 Quieren migrar a un rol de **IA Engineer**, **Data Scientist**, **Data Engineer** o **Data Analyst**
- 💻 Son **principiantes en IA** o **desarrolladores junior** (25-35 años)
- 🛠️ Prefieren **aprender haciendo** en lugar de solo ver cursos
- 🚀 Buscan construir un **portafolio profesional** con proyectos reales

## 💡 Filosofía del Proyecto

> **Crear es mejor que ver cursos**

Estos proyectos están diseñados bajo el principio de que la mejor manera de aprender es **construyendo**. Cada tutorial te guía paso a paso para que:

1. Entiendas **qué** estás haciendo
2. Comprendas **por qué** lo estás haciendo
3. Tengas un **proyecto funcional** al terminar
4. Puedas **mostrarlo en tu portafolio**

## 📚 Tabla de Proyectos

| Nombre del Proyecto                           | Dificultad      | Link                                         |
| --------------------------------------------- | --------------- | -------------------------------------------- |
| 🤖 Agente RAG - Asistente que Lee PDFs        | 🟢 Principiante | [Ver Tutorial](./01-pdf-rag-agent/) |
| 💬 Chatbot con Memoria Conversacional         | 🟡 Intermedio   | 🔜 Próximamente                              |
| 🖼️ Clasificador de Imágenes con CNN           | 🟡 Intermedio   | 🔜 Próximamente                              |
| 😊 Análisis de Sentimientos en Redes Sociales | 🟢 Principiante | 🔜 Próximamente                              |
| 🎯 Sistema de Recomendaciones                 | 🟡 Intermedio   | 🔜 Próximamente                              |
| 📊 Dashboard de Análisis de Datos             | 🟢 Principiante | 🔜 Próximamente                              |
| 🎤 Transcriptor de Audio con Whisper          | 🟢 Principiante | 🔜 Próximamente                              |
| 🔗 API REST con FastAPI + IA                  | 🟡 Intermedio   | 🔜 Próximamente                              |
| 🧠 Fine-tuning de Modelos de Lenguaje         | 🔴 Avanzado     | 🔜 Próximamente                              |
| 🔍 Buscador Semántico con Embeddings          | 🟡 Intermedio   | 🔜 Próximamente                              |

---

## 🛠️ Tecnologías que Usarás

A lo largo de estos proyectos aprenderás a trabajar con:

### Lenguajes

![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![LangChain](https://img.shields.io/badge/LangChain-121212?style=flat&logo=chainlink&logoColor=white)
![OpenAI](https://img.shields.io/badge/OpenAI-412991?style=flat&logo=openai&logoColor=white)
![HuggingFace](https://img.shields.io/badge/HuggingFace-FFD21E?style=flat&logo=huggingface&logoColor=black)
![ChromaDB](https://img.shields.io/badge/ChromaDB-FF6F00?style=flat)
![Pinecone](https://img.shields.io/badge/Pinecone-000000?style=flat)
![uv](https://img.shields.io/badge/uv-DE5FE9?style=flat)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)

---

## ⭐ Apoya Este Proyecto

Si estos tutoriales te ayudaron:

- ⭐ Dale una estrella a este repositorio
- 🔄 Compártelo con otros que están aprendiendo
- 📢 Menciona tus logros en redes sociales con **#AprendiendoIA**
- 🎥 Suscríbete al canal de YouTube para más contenido

---

## 💪 Recuerda

> **"La mejor manera de predecir el futuro es construyéndolo"**

No esperes a sentirte "listo". **Empieza ahora**. Cada proyecto que completes te acerca más a tu objetivo de trabajar en IA.

**Crear > Consumir**

**¿Listo para empezar?** 👉 [Comienza con el Agente RAG](./01-agente-rag/README.md)

---

<p align="center">
  Hecho con ❤️ para la comunidad de IA en español
</p>

<p align="center">
  <sub>Si este proyecto te ayudó, considera darle una ⭐</sub>
</p>
